...
namespace Problem11 {
	class Program {
		static UInt64 largestProduct = 0;
		static UInt64 currentResult;
		static void UpdateLargestProduct() {
			if(currentResult > largestProduct)
				largestProduct = currentResult;
		}
		...
		static void Main(string[] args) {
			...
			for(int i = 0; i < grid.Count(); i++) {
				for(int j = 0; j < grid[i].Count(); j++) {
					if(j + 3 < grid[i].Count()) {
						currentResult = (UInt64)grid[i][j] * (UInt64)grid[i][j + 1] * (UInt64)grid[i][j + 2] * (UInt64)grid[i][j + 3];
						UpdateLargestProduct();

					}
					if(i + 3 < grid.Count()) {
						currentResult = (UInt64)grid[i][j] * (UInt64)grid[i + 1][j] * (UInt64)grid[i + 2][j] * (UInt64)grid[i + 3][j];
						UpdateLargestProduct();
						if(j - 3 >= 0) {
							currentResult = (UInt64)grid[i][j] * (UInt64)grid[i + 1][j - 1] * (UInt64)grid[i + 2][j - 2] * (UInt64)grid[i + 3][j - 3];
							UpdateLargestProduct();
						}
						if(j + 3 < grid[i].Count()) {
							currentResult = (UInt64)grid[i][j] * (UInt64)grid[i + 1][j + 1] * (UInt64)grid[i + 2][j + 2] * (UInt64)grid[i + 3][j + 3];
